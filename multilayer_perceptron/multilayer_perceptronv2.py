...
def run(iterations, print_freq,logfile):
    ...
    for i in range(iterations):
        ...
        r = np.random.randint(0,num_images,batch_size)
        l1 = 0
        l2 = 0
        out = 0
        ...
        for h in range(batch_size):
            ...
            l1 += l1_delta(inputs[r][h], y_pred[r][0])
            
            l2 += l2_delta(inputs[r][h], y_pred[r][0])
            
            out += out_delta(inputs[r][h], y_pred[r][0])
...
for learning_rate in learning_rates:
    for batch_size in batch_sizes:
        ...
        if((batch_size == 50 or batch_size == 100) and (learning_rate == 0.01 or learning_rate == 0.1)):
            continue

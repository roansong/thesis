...
IMG_HEIGHT = 40 # biggest picture in the set
IMG_WIDTH = 40
...
for n in range(filedata['filename'].shape[0]):
    ...
    if(IN_HEIGHT < IMG_HEIGHT):
        in_img = pad_img(in_img,IMG_HEIGHT,IMG_WIDTH,IN_HEIGHT,IN_WIDTH)
    else:
        in_img = crop_img(in_img,IMG_HEIGHT,IMG_WIDTH,IN_HEIGHT,IN_WIDTH)
    ...
    for k in range(filedata['filename'].shape[0]):
        ...
        if(CUR_HEIGHT < IMG_HEIGHT):
            ...
            cur_img = crop_img(cur_img,IMG_HEIGHT,IMG_WIDTH,CUR_HEIGHT,CUR_WIDTH)
